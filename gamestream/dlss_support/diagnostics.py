...
DIRECT_FEATURE_18_MARKERS = (
    ("direct host ready", re.compile(r"DLSS5 host ready:")),
    (
        "feature 18 created",
        re.compile(r"Feature18 CreateFeature\(18\) -> 0x00000001"),
    ),
    (
        "feature 18 evaluated",
        re.compile(r"Feature18 EvaluateFeature succeeded"),
    ),
)
...
def inspect_bundle(layout: RuntimeLayout) -> dict[str, Any]:
    ...
    addon = file_sha256(layout.addon) if layout.addon.is_file() else "not used on Linux"
    neural = file_sha256(layout.neural_runtime)
    direct_linux = True  # Scry uses the direct bridge on both platforms.
    return {
        "addon_sha256": addon,
        "neural_sha256": neural,
        "known_ampere_pair": (
            neural == EXPECTED_AMPERE_NEURAL_SHA256
            and (direct_linux or addon == EXPECTED_AMPERE_ADDON_SHA256)
        ),
    }
...
def ensure_supported(layout: RuntimeLayout) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if gpu["generation"] == 30 and not bundle["known_ampere_pair"]:
        requirement = "tested DLSS NR 310.8.SF-v2 runtime"
        raise RuntimeError(
            f"{gpu['name']} needs the {requirement}. Installed hashes are "
...
def verify_direct_feature_18(worker_logs: list[str], frames: int) -> dict[str, Any]:
    """Confirm the Linux direct bridge created and evaluated signed feature 18."""
    text = "\n".join(worker_logs)
    missing = [name for name, pattern in DIRECT_FEATURE_18_MARKERS if not pattern.search(text)]
    if frames <= 0:
        missing.append("a returned neural-rendered frame")
    if missing:
        raise RuntimeError(
            "The frames were processed, but the Linux bridge did not prove signed "
            "DLSSNR feature-18 execution. Missing evidence:\n  "
            + "\n  ".join(missing)
            + ("\n\n" + "\n".join(worker_logs[-40:]) if worker_logs else "")
        )
    evidence = [
        line
        for line in worker_logs
        if "DLSS5 host ready:" in line
        or "Feature18 CreateFeature" in line
        or "Feature18 EvaluateFeature succeeded" in line
        or "DLSS carrier CreateFeature" in line
    ]
    return {
        "verified": True,
...
def describe_worker_failure(
    *,
    ...
    headline = (
        "The DLSS worker crashed inside feature-18 evaluation "
        f"(access violation) on frame {frame_index}."
        if access_violation
        else f"The DLSS worker stopped on frame {frame_index} (exit {exit_code})."
    )
    hint = ""
