...
static bool g_neural_first = false;
...
static NGXResult __cdecl ScalingRatioCallback(NGXParameter* parameters) {
    ...
    // ordinary DLSS carrier performs the enlargement separately.
...
static bool AllocateFrameResources(UINT input_w, UINT input_h, UINT output_w, UINT output_h) {
    ...
    if (g_upscale_active) {
        g_dlss_output = CreateTexture(g_neural_first ? input_w : output_w, g_neural_first ? input_h : output_h, DXGI_FORMAT_R16G16B16A16_FLOAT,
...
static void SetDLSSCarrierParams(int perf_quality, int reset) {
    ...
    // kept separate from DLSSNR.* aliases: render-sized Color is either the
    // captured image or the neural-first intermediate; Output is target-sized.
    ...
    SetParamResource("Color", g_neural_first ? g_dlss_output.Get() : g_color.Get());
    SetParamResource("MotionVectors", g_mvec.Get());
    SetParamResource("Depth", g_depth.Get());
    SetParamResource("Output", g_neural_first ? g_output.Get() : g_dlss_output.Get());
}
...
static void SetNeuralParams(
    ...
    // Neural rendering preserves spatial size. It operates at input resolution
    // in neural-first mode, or target resolution after the DLSS carrier.
    const UINT neural_color_width = g_upscale_active && !g_neural_first ? g_output_width : g_input_width;
    const UINT neural_color_height = g_upscale_active && !g_neural_first ? g_output_height : g_input_height;
    const int neural_quality = g_neural_first ? 5 : (g_upscale_active ? NR_POSTPASS_PERF_QUALITY : perf_quality);

    SetParamUInt("DLSSNR.Width", neural_color_width);
    SetParamUInt("DLSSNR.Height", neural_color_height);
    SetParamUInt("DLSSNR.InputWidth", neural_color_width);
    SetParamUInt("DLSSNR.InputHeight", neural_color_height);
    SetParamUInt("DLSSNR.OutputWidth", neural_color_width);
    SetParamUInt("DLSSNR.OutputHeight", neural_color_height);
    SetParamUInt("DLSSNR.Output.Width", neural_color_width);
    SetParamUInt("DLSSNR.Output.Height", neural_color_height);
    ...
    ID3D12Resource* color = g_upscale_active && !g_neural_first ? g_dlss_output.Get() : g_color.Get();
    SetParamResource("DLSSNR.Color", color);
    SetParamResource("DLSSNR.MVec", g_mvec.Get());
    SetParamResource("DLSSNR.Depth", g_depth.Get());
    SetParamResource("DLSSNR.Output", g_upscale_active && g_neural_first ? g_dlss_output.Get() : g_output.Get());
    SetParamResource("DLSSNR.Backbuffer", g_upscale_active && g_neural_first ? g_dlss_output.Get() : g_output.Get());
    ...
    SetParamUInt("DLSSNR.OutputSubrectWidth", neural_color_width);
    SetParamUInt("DLSSNR.OutputSubrectHeight", neural_color_height);
}
...
static bool LoadNGX() {
    ...
    wchar_t configured_shim[32768] = {};
    const DWORD configured_shim_len = GetEnvironmentVariableW(
        L"DLSS5NR_CALLER_SHIM", configured_shim,
        static_cast<DWORD>(std::size(configured_shim)));
    std::wstring shim_path;
    if (configured_shim_len > 0 && configured_shim_len < std::size(configured_shim)) {
        shim_path.assign(configured_shim, configured_shim_len);
    } else {
...
__declspec(dllexport) int __cdecl dlss5nr_init(int gpu_index, const wchar_t* runtime_dir, char* err, int err_cap) {
    ...
    char order[8] = {};
    GetEnvironmentVariableA("GAMESTREAM_NEURAL_FIRST", order, sizeof(order));
    g_neural_first = std::strcmp(order, "1") == 0;
    std::fprintf(stderr, "[dlss5nr] Pass order: %s\n", g_neural_first ? "neural -> DLSS" : "DLSS -> neural");
...
static int ProcessFrame(
    const float* rgb_in, const uint8_t* rgba_in, const uint16_t* mvec_in,
    float* rgb_out, uint8_t* rgba_out,
    ...
    if ((!rgb_in && !rgba_in) || !mvec_in || (!rgb_out && !rgba_out) ||
    ...
    for (int y = 0; y < input_height; ++y) {
        ...
        if (rgba_in) {
            const uint8_t* src = rgba_in + static_cast<size_t>(y) * input_width * 4;
            static uint16_t to_half[256];
            static bool initialized = false;
            if (!initialized) {
                for (int i = 0; i < 256; ++i) to_half[i] = FloatToHalf(i / 255.0f);
                initialized = true;
            }
            for (int x = 0; x < input_width; ++x) {
                row[x * 4 + 0] = to_half[src[x * 4 + 0]];
                row[x * 4 + 1] = to_half[src[x * 4 + 1]];
                row[x * 4 + 2] = to_half[src[x * 4 + 2]];
    ...
    // The intermediate is input-sized for neural-first and target-sized otherwise.
    if (g_upscale_active && g_dlss_output_readable) {
        auto writable = Barrier(g_dlss_output.Get(), D3D12_RESOURCE_STATE_NON_PIXEL_SHADER_RESOURCE,
                                D3D12_RESOURCE_STATE_UNORDERED_ACCESS);
        g_cmd->ResourceBarrier(1, &writable);
        g_dlss_output_readable = false;
    }
    const auto evaluate_neural = [&]() {
        SetNeuralParams(style, preset, perf_quality, intensity, tone, structure, skin, automask, reset ? 1 : 0);
        return g_shim_eval(reinterpret_cast<void*>(g_nr_eval), g_cmd.Get(), g_feature, g_params, nullptr);
    };
    const auto evaluate_dlss = [&]() {
        SetDLSSCarrierParams(perf_quality, reset ? 1 : 0);
        return g_core_eval(g_cmd.Get(), g_dlss_feature, g_params, nullptr);
    };
    NGXResult er = g_upscale_active && !g_neural_first ? evaluate_dlss() : evaluate_neural();
    if (er != NGX_SUCCESS) {
        SetError("First rendering pass failed: 0x%08X", static_cast<unsigned>(er));
        ExecuteAndWait(); CopyError(err, err_cap); return 0;
    }
    if (g_upscale_active) {
        D3D12_RESOURCE_BARRIER ready[2]{};
        ready[0].Type = D3D12_RESOURCE_BARRIER_TYPE_UAV;
        ready[0].UAV.pResource = g_dlss_output.Get();
        ready[1] = Barrier(g_dlss_output.Get(), D3D12_RESOURCE_STATE_UNORDERED_ACCESS,
                           D3D12_RESOURCE_STATE_NON_PIXEL_SHADER_RESOURCE);
        g_cmd->ResourceBarrier(2, ready);
        g_dlss_output_readable = true;
        er = g_neural_first ? evaluate_dlss() : evaluate_neural();
        if (er != NGX_SUCCESS) {
            SetError("Second rendering pass failed: 0x%08X", static_cast<unsigned>(er));
            ExecuteAndWait(); CopyError(err, err_cap); return 0;
        }
    }
    ...
    for (int y = 0; y < output_height; ++y) {
        ...
        if (rgba_out) {
            // Exact bounded lookup replaces millions of scalar half-float
            // conversions. ProcessFrame holds g_mutex during initialization.
            static uint8_t to_byte[65536];
            static bool initialized = false;
            if (!initialized) {
                for (int i = 0; i < 65536; ++i) {
                    float value = HalfToFloat(static_cast<uint16_t>(i));
                    if (std::isnan(value)) value = 0.0f;
                    to_byte[i] = static_cast<uint8_t>(std::clamp(value, 0.0f, 1.0f) * 255.0f + 0.5f);
                }
                initialized = true;
            }
            uint8_t* dst = rgba_out + static_cast<size_t>(y) * output_width * 4;
            for (int x = 0; x < output_width; ++x) {
                for (int channel = 0; channel < 3; ++channel) {
                    dst[x * 4 + channel] = to_byte[row[x * 4 + channel]];
                }
                dst[x * 4 + 3] = 255;
            }
        } else {
...
__declspec(dllexport) int __cdecl dlss5nr_process_v2(
    ...
    float intensity, float tone, float structure, float skin,
    int automask, int reset, char* err, int err_cap) {
    return ProcessFrame(rgb_in, nullptr, mvec_in, rgb_out, nullptr,
                        input_width, input_height, output_width, output_height,
                        style, preset, perf_quality, intensity, tone, structure, skin,
                        automask, reset, err, err_cap);
}

// Zero-copy-friendly transport for live capture. The caller's RGBA8 buffers
// are converted directly to/from the mapped D3D12 surfaces, avoiding two large
// RGB32F intermediates for every frame.
__declspec(dllexport) int __cdecl dlss5nr_process_rgba8_v3(
    const uint8_t* rgba_in, const uint16_t* mvec_in, uint8_t* rgba_out,
    int input_width, int input_height, int output_width, int output_height,
    int style, int preset, int perf_quality,
    float intensity, float tone, float structure, float skin,
    int automask, int reset, char* err, int err_cap) {
    return ProcessFrame(nullptr, rgba_in, mvec_in, nullptr, rgba_out,
                        input_width, input_height, output_width, output_height,
                        style, preset, perf_quality, intensity, tone, structure, skin,
                        automask, reset, err, err_cap);
}
...
__declspec(dllexport) int __cdecl dlss5nr_process(
    ...
    return ProcessFrame(rgb_in, nullptr, zero_motion.data(), rgb_out, nullptr,
